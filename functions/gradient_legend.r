 ...
 gradient.legend <- function(data, side=4, colours, scale=1, distance=0, label="", xpd=NA, cex=1, col.axis=par("col.axis"), yjust=0, ...){
...
# cex.old = par()$cex
 ...
 if(side==4){
  x.lim <- par("usr")[2]*((distance*0.08)+1.05)
    ...
    axis(side, pos=x.lim,      at=yjust+seq(par("usr")[4]-(1-scale)*diff(c(par("usr")[3],par("usr")[4])), par("usr")[3]+(1-scale)*diff(c(par("usr")[3],par("usr")[4])),length=11), labels=signif(seq(max(data), min(data), length=11),2), padj=-1, cex.axis=cex, col.axis=col.axis, col.ticks=col.axis, col=col.axis, col.lab=col.axis, xpd=NA, ...)
