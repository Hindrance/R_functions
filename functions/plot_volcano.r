...
plot.volcano_v1 <- function(data, input.format, colour1, colour2, fold.change, p.level, basemean.cut, text, fade_style, lines, plot.legend, legendPosition, cex, xlab, ylab, new.plot=T,...) {
...
if(missing(text)){text = F} else {do_you_want_text = text}
...
# Initialise plot
	par(cex=cex); if(new.plot==T) {plot(c(min(neg[,2])+min(neg[,2])/15,max(pos[,2])+max(pos[,2])/15),c(0,max(neg[,3],pos[,3])+max(neg[,3],pos[,3])/15), pch=NA, col="white", xlab=xlab, ylab=ylab,...)}
...
# text?
	...
	if(length(x[res.vec]) != 0){text(txt.norm.x[res.vec], txt.norm.y[res.vec], neg[res.vec,1])}}
...
# plot significant only
			if(fade_style <= 2){points(x[res.vec], y[res.vec], pch=21, bg=rgb(n.col[1]/255,n.col[2]/255,n.col[3]/255,1), col="black", lwd=2)} else {
                                                        points(x[res.vec], y[res.vec], pch=21, bg=cols[res.vec], col="black", lwd=2)}
}
...
# plot significant only
			if(fade_style <= 2){points(x[res.vec], y[res.vec], pch=21, bg=rgb(p.col[1]/255,p.col[2]/255,p.col[3]/255,1), col="black", lwd=2)} else {
                                points(x[res.vec], y[res.vec], pch=21, bg=cols[res.vec], col="black", lwd=2)}
...
# Legend?
	...
	if(plot.legend == T){
	par(lend=2); legend(legendPosition, legend=c("Reduced expression", "Increased expression"), pt.bg=c(rgb(n.col[1]/255,n.col[2]/255,n.col[3]/255,1), rgb(p.col[1]/255,p.col[2]/255,p.col[3]/255,1)), lty=0, pch=21, lwd=1, bty="n")
	}
}
