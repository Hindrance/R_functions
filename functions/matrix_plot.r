...
matrix.plot <- function(matrix2plot, col.scale=Heat, cex.multiplier=3, y.axis=T, x.axis=T, pch=15, cex.axis=1,  round.n = "default", ...){ 
...
# Colour matrix
  ...
  # make initial plot
    plot(c(1, dim(matrix2plot)[2])+2, c(1, dim(matrix2plot)[1]), pch=NA, ... , xaxt="n", yaxt="n", xaxs="i", yaxs="i", ylim=c(0,dim(matrix2plot)[1]+1), xlim=c(0,dim(matrix2plot)[2]+1), bty="n", asp=T)
